...
const Card: React.FC<CardProps> = ({ card, view }) => {
  ...
  return (
    <Link href={`/posts/${card.slug}`} key={card.slug}>
      <li
        className={`flex ${isListView ? 'flex-row max-w-[780px] mx-auto items-center' : 'flex-col'} rounded-lg items-start p-4 hover:bg-gradient-to-r from-slate-900 to-slate-800 w-full h-full`}
      >
        <div
