...
export const renderDescription = (description: DescriptionItem[]) => {
  return description.map((item) => {
    if (item.type === 'p') {
      return (
        <p key={item.id} className="text-slate-500 dark:text-gray-300 mb-4">
          {item.text}
        </p>
      );
      ...
      return (
        <ul
          key={item.id}
          className="list-disc list-inside text-slate-500 dark:text-gray-300 mb-4"
        >
...
export const AboutSectionCard = ({ sections }: CardProps) => {
  return (
    <div className="bg-gradient-to-r from-slate-300 to-slate-400 dark:from-zinc-700 dark:to-zinc-600 shadow-lg rounded-lg p-6 flex flex-col gap-2 max-w-[740px]">
